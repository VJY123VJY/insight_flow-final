...
class ObservabilityService:
    ...
    async def log_to_bucket(decision: str, reason: str, request_data: Dict[str, Any], payload: Dict[str, Any]):
        ...
        Handles network errors and timeouts gracefully without crashing enforcement flow.
        """
        audit_entry = {
        ...
        timeout_seconds = getattr(settings, "BUCKET_TIMEOUT", 3.0)
        headers = {"Content-Type": "application/json"}
        if getattr(settings, "BUCKET_AUTH_TOKEN", None):
            headers["Authorization"] = f"Bearer {settings.BUCKET_AUTH_TOKEN}"
            
        try:
            async with httpx.AsyncClient(timeout=float(timeout_seconds)) as client:
                response = await client.post(settings.BUCKET_ENDPOINT, json=audit_entry, headers=headers)
                if response.is_success:
                    logger.info(
                        f"Audit event successfully posted to Bucket: {audit_entry['audit_id']} "
                        f"(HTTP {response.status_code})"
                    )
                else:
                    logger.error(
                        f"Bucket rejected audit event {audit_entry['audit_id']}: "
                        f"HTTP {response.status_code} - {response.text}"
                    )
        except httpx.TimeoutException as te:
            logger.error(
                f"Bucket connection timed out after {timeout_seconds}s for audit {audit_entry['audit_id']}: {te}"
            )
        except httpx.ConnectError as ce:
            logger.error(
                f"Failed to connect to Bucket endpoint ({settings.BUCKET_ENDPOINT}): {ce}"
            )
        except Exception as e:
